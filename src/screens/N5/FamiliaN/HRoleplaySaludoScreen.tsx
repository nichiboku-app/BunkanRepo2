...
/* =========================================================
   Hints y trazos (para el frame)
========================================================= */
type XY = { x: number; y: number };
const HINTS: Record<string, XY[]> = {
  'は': [{ x: 0.4, y: 0.2 }, { x: 0.62, y: 0.22 }, { x: 0.56, y: 0.6 }],
  'ひ': [{ x: 0.55, y: 0.45 }],
  'ふ': [{ x: 0.42, y: 0.18 }, { x: 0.58, y: 0.18 }, { x: 0.52, y: 0.62 }],
  'へ': [{ x: 0.35, y: 0.55 }],
  'ほ': [{ x: 0.4, y: 0.2 }, { x: 0.62, y: 0.2 }, { x: 0.6, y: 0.58 }, { x: 0.5, y: 0.64 }],
  'こ': [{ x: 0.35, y: 0.3 }, { x: 0.65, y: 0.65 }],
  'ん': [{ x: 0.55, y: 0.45 }],
  'に': [{ x: 0.35, y: 0.3 }, { x: 0.65, y: 0.65 }],
  'ち': [{ x: 0.4, y: 0.3 }, { x: 0.55, y: 0.65 }],
  'お': [{ x: 0.4, y: 0.25 }, { x: 0.4, y: 0.55 }, { x: 0.62, y: 0.62 }],
  'よ': [{ x: 0.45, y: 0.35 }, { x: 0.6, y: 0.6 }],
  'う': [{ x: 0.55, y: 0.4 }, { x: 0.5, y: 0.65 }],
  'い': [{ x: 0.45, y: 0.3 }, { x: 0.6, y: 0.55 }],
  'ま': [{ x: 0.35, y: 0.3 }, { x: 0.6, y: 0.3 }, { x: 0.5, y: 0.6 }],
  'す': [{ x: 0.45, y: 0.3 }, { x: 0.55, y: 0.65 }],
  'ろ': [{ x: 0.52, y: 0.5 }],
  'く': [{ x: 0.55, y: 0.45 }],
  'ね': [{ x: 0.45, y: 0.3 }, { x: 0.6, y: 0.55 }],
  'し': [{ x: 0.55, y: 0.45 }],
  'て': [{ x: 0.45, y: 0.3 }, { x: 0.58, y: 0.55 }],
  'ご': [{ x: 0.35, y: 0.3 }, { x: 0.65, y: 0.65 }, { x: 0.78, y: 0.2 }],
  'ざ': [{ x: 0.35, y: 0.18 }, { x: 0.38, y: 0.38 }, { x: 0.68, y: 0.62 }, { x: 0.78, y: 0.18 }],
  'じ': [{ x: 0.52, y: 0.35 }, { x: 0.78, y: 0.18 }],
  'が': [{ x: 0.42, y: 0.2 }, { x: 0.58, y: 0.22 }, { x: 0.56, y: 0.6 }, { x: 0.78, y: 0.18 }],
};

const STROKE_COUNT: Record<string, number> = {
  'は': 3, 'ひ': 1, 'ふ': 3, 'へ': 1, 'ほ': 4,
  'こ': 2, 'ん': 1, 'に': 2, 'ち': 2, 'お': 3, 'よ': 2, 'う': 2, 'い': 2,
  'ま': 3, 'す': 2, 'ろ': 1, 'く': 1, 'ね': 2, 'し': 1, 'て': 2,
  'ご': 3, 'ざ': 4, 'じ': 2, 'が': 4,
};

/* =========================================================
   SFX (correcto/incorrecto) con expo-audio
========================================================= */
function useSfx() {
  ...
  const playOk = useCallback(async () => {
    Vibration.vibrate(8);
  ...
  const playBad = useCallback(async () => {
    Vibration.vibrate(8);
    await setAudioModeAsync({ playsInSilentMode: true, allowsRecording: false });
    bad.seekTo(0);
    bad.play();
...
export default function HRoleplaySaludoScreen() {
  ...
  // Precarga de URIs locales de frases
  const [phraseUris, setPhraseUris] = useState<Partial<Record<PhraseItem['id'], string>>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const out: Partial<Record<PhraseItem['id'], string>> = {};
        for (const id of Object.keys(PHRASE_AUDIO) as PhraseItem['id'][]) {
          const asset = Asset.fromModule(PHRASE_AUDIO[id]);
          await asset.downloadAsync();
          out[id] = asset.localUri || asset.uri;
        }
        if (!cancelled) setPhraseUris(out);
      } finally {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      <Text style={styles.subtitle}>
        Arriba tienes el marco de trazos para la <Text style={{ fontWeight: '800' }}>familia H</Text>. Toca una letra o
        un chip de las frases para ver su orden sugerido.
      </Text>
...
/* ====== Fila de frase con reproductor propio ====== */
function PhraseRow({
  item,
  localUri,
  ready,
      ...
      <View style={styles.kanaWrap}>
        {item.kanaBreak.map((k, idx) => (
          <LetterChip
            ...
            part={k}
            onOpen={() => { /* sincroniza desde el padre vía chips; aquí solo UI */ }}
          />
        ))}
      </View>
    </View>
...
/* ====== Chip de letra con modal ====== */
function LetterChip({ part, onOpen }: { part: KanaPart; onOpen: () => void }) {
  ...
  return (
    <>
      <Pressable
        onPress={() => { Vibration.vibrate(6); onOpen(); setOpen(true); }}
