...
const PATRONES_RANGE: Row[] = [
  {
    icon: "time-outline",
    titulo: "Horas exactas",
    ejemplos: ["9じ から 5じ まで"],
    regla: "Marca el inicio (から) y el fin (まで) de un horario.",
  },
  {
    icon: "calendar-outline",
    titulo: "Fechas / días",
    ejemplos: ["3がつ3にち から 3がつ5にち まで", "げつようび から きんようび まで"],
    regla: "Rango de días concretos del calendario.",
  },
  {
    icon: "calendar-number-outline",
    titulo: "Años y meses",
    ejemplos: ["2024ねん から 2025ねん まで", "3がつ から 4がつ まで"],
    regla: "Periodos más largos (meses / años).",
  },
  {
    icon: "navigate-outline",
    titulo: "Lugares",
    ejemplos: ["いえ から かいしゃ まで", "えき から がっこう まで"],
    regla: "Trayecto de un lugar a otro.",
  },
  {
    icon: "list-outline",
    titulo: "Números",
    ejemplos: ["1 から 10 まで"],
    regla: "Rango numérico “de… a…”.",
  },
];
...
const Q: TokenQuiz[] = [
  {
    id: 1,
    promptEs: "Arma: «Trabajo de 9 a 5.»",
    hintJp: "9じ / 5じ / しごと / します",
    tokens: ["5じ まで", "します", "9じ から", "しごと を"],
    answers: ["9じ から 5じ まで しごと を します"],
    explain: "Rango temporal: 9じ から 5じ まで + acción.",
  },
  {
    id: 2,
    promptEs: "Arma: «Voy a la escuela de lunes a viernes.»",
    hintJp: "げつようび / きんようび / がっこう / いきます",
    tokens: ["きんようび まで", "いきます", "げつようび から", "がっこう に"],
    answers: ["げつようび から きんようび まで がっこう に いきます"],
    explain: "Días concretos → から／まで.",
  },
  {
    id: 3,
    promptEs: "Arma: «El festival es del 3 al 5 de marzo.»",
    hintJp: "まつり / 3がつ3にち / 3がつ5にち / です",
    tokens: ["3がつ3にち から", "です", "3がつ5にち まで", "まつり は"],
    answers: ["まつり は 3がつ3にち から 3がつ5にち まで です"],
    explain: "Tema + rango + です.",
  },
  {
    id: 4,
    promptEs: "Arma: «Desde hoy hasta mañana descanso.»",
    hintJp: "きょう / あした / やすみます",
    tokens: ["きょう から", "やすみます", "あした まで"],
    answers: ["きょう から あした まで やすみます"],
    explain: "Palabras relativas (きょう・あした) también usan から／まで.",
  },
  {
    id: 5,
    promptEs: "Arma: «La biblioteca abre de 10 a 18.»",
    hintJp: "としょかん / 10じ / 18じ / あいています",
    tokens: ["18じ まで", "としょかん は", "あいています", "10じ から"],
    answers: ["としょかん は 10じ から 18じ まで あいています"],
    explain: "Entidad + rango + estado (あいています).",
  },
  {
    id: 6,
    promptEs: "Arma: «La clase es de 2 a 4.»",
    hintJp: "クラス / 2じ / 4じ / です",
    tokens: ["クラス は", "2じ から", "4じ まで", "です"],
    answers: ["クラス は 2じ から 4じ まで です"],
    explain: "X は A から B まで です.",
  },
  {
    id: 7,
    promptEs: "Arma: «Estamos de vacaciones del 1 al 7 de agosto.»",
    hintJp: "8がつ1にち / 7にち / やすみ",
    tokens: ["8がつ1にち から", "やすみ です", "7にち まで"],
    answers: ["8がつ1にち から 7にち まで やすみ です"],
    explain: "Fechas concretas → から／まで.",
  },
  {
    id: 8,
    promptEs: "Arma: «Cuento del 1 al 10.»",
    hintJp: "1 / 10 / かぞえます",
    tokens: ["10 まで", "かぞえます", "1 から"],
    answers: ["1 から 10 まで かぞえます"],
    explain: "Rango numérico.",
  },
  {
    id: 9,
    promptEs: "Arma: «Voy desde casa hasta la empresa.»",
    hintJp: "いえ / かいしゃ / いきます",
    tokens: ["かいしゃ まで", "いえ から", "いきます"],
    answers: ["いえ から かいしゃ まで いきます"],
    explain: "Rango de lugar.",
  },
  {
    id: 10,
    promptEs: "Arma: «De 6 a 7 estudio japonés.»",
    hintJp: "6じ / 7じ / にほんご / べんきょう します",
    tokens: ["にほんご を", "7じ まで", "6じ から", "べんきょう します"],
    answers: ["6じ から 7じ まで にほんご を べんきょう します"],
    explain: "Rango + objeto を + verbo.",
  },
];
...
export default function B5_TiempoDuracion() {
  ...
  const onUndo = (q: TokenQuiz) => {
    setAns((prev) => {
      const cur = prev[q.id];
      if (!cur || cur.order.length === 0 || cur.correct) return prev;
  ...
  const onReset = (q: TokenQuiz) => {
    setAns((prev) => ({
      ...prev,
      [q.id]: { used: Array(q.tokens.length).fill(false), order: [] },
    }));
  };
        ...
        <ScrollView contentContainerStyle={s.c} nestedScrollEnabled>
          ...
          <View style={s.tableWrap}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator contentContainerStyle={{ paddingBottom: 6 }}>
              <View style={[s.table, { minWidth: 780 }]}>
                ...
                {PATRONES_RANGE.map((r, i) => (
                  <View key={i} style={[s.tr, i % 2 ? s.trAlt : null]}>
                    ...
                    <View style={[s.tdBox, s.colEj]}>
                      <Text style={s.tdText}>{r.ejemplos.join(" ／ ")}</Text>
                    </View>
...
const s = StyleSheet.create({
  ...
  colEj: { width: 340, paddingHorizontal: 6 },
  colRegla: { width: 260, paddingHorizontal: 6 },

  tdBox: { justifyContent: "center" },
  tdText: { color: INK, fontSize: 14, lineHeight: 18 },
