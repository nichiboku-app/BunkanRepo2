...
// ----------------------------------------------------
// 1. IMPORTACIÓN DEL ARCHIVO DE PREGUNTAS
// Asumiendo que RetoN2Panda.ts está en: src/data/RetoN2Panda.ts
// La ruta relativa debe ser ajustada a tu estructura de carpetas.
// (Aquí asumo que RetoN2.tsx está en src/screens/RetoN2.tsx)
import { questionsN2 } from '../data/RetoN2Panda';
// ----------------------------------------------------

const { width, height } = Dimensions.get('window');

// 🐼 Panda base y estados
const pandaBase = require('../../assets/panda/panda_move_7.png'); // idle / manos juntas
const pandaFail = require('../../assets/panda/panda_error.png');
const pandaHappy = require('../../assets/panda/panda_exito.png');

// 🐼 Frames de caminata
const pandaWalkFrames = [
    require('../../assets/panda/panda_move_1.png'),
    require('../../assets/panda/panda_move_2.png'),
    require('../../assets/panda/panda_move_3.png'),
    require('../../assets/panda/panda_move_4.png'),
    require('../../assets/panda/panda_move_5.png'),
    require('../../assets/panda/panda_move_6.png'),
    require('../../assets/panda/panda_move_7.png'),
];

// Fondo y timer
const backgroundPanda = require('../../assets/panda/pandagame.png');
// si tienes un timer propio, cambia esta ruta:
const timerImage = require('../../assets/leon/timerleon.png');

// Música del nivel
const bgmPanda = require('../../assets/sounds/pandagame.mp3');
...
// ---------- CONFIG NIVELES/FASES (N2) ----------
// Adaptado para 5 fases x 40 preguntas = 200 preguntas (goi, bunpou, moji, dokkai, gengochishiki)

export const LEVELS_N2: LevelConfig[] = [
    {
        id: 1,
        name: 'Nivel N2 · Dojo del Panda',
        phases: [
            ...
            { id: 5, name: 'Fase 5 · Expresiones y Uso (Gengochishiki)' },
        ],
    },
];

// Ajustado a 40 preguntas por fase para cubrir las 5 secciones de 40 preguntas cada una
const QUESTIONS_PER_PHASE = 40; 
const TOTAL_PHASES_N2 = LEVELS_N2.reduce(
    (sum, level) => sum + level.phases.length,
    0
);

// ⚠️ Se eliminaron las preguntas demo. El array 'questionsN2' ahora es el importado.

// ===================== COMPONENTE =====================

export default function RetoN2() {
    ...
    const [pandaState, setPandaState] = useState<'normal' | 'fail' | 'happy'>(
        'normal'
    );
    ...
    const currentLevel = LEVELS_N2[currentLevelIndex];
    const currentPhase = currentLevel.phases[currentPhaseIndex];

    const globalPhaseIndex =
        LEVELS_N2.slice(0, currentLevelIndex).reduce(
            (sum, level) => sum + level.phases.length,
            0
        ) + currentPhaseIndex;

    const phaseStartIndex = globalPhaseIndex * QUESTIONS_PER_PHASE;
    const phaseQuestions = questionsN2.slice(
        phaseStartIndex,
        phaseStartIndex + QUESTIONS_PER_PHASE
    );

    const isGameOutOfQuestions = phaseStartIndex >= questionsN2.length;
    ...
    const isLastPhaseOfGame =
        globalPhaseIndex === TOTAL_PHASES_N2 - 1 ||
        phaseStartIndex + QUESTIONS_PER_PHASE >= questionsN2.length;
    ...
    useEffect(() => {
        ...
        (async () => {
            const { sound } = await Audio.Sound.createAsync(bgmPanda);
            if (!mounted) {
    ...
    const handleNextPhase = async (success: boolean) => {
        if (success) {
            ...
            if (nextPhaseIndex < currentLevel.phases.length) {
                ...
                if (nextLevelIndex < LEVELS_N2.length) {
            ...
            if (isLastPhaseOfGame) {
                setTimeout(async () => {
                    ...
                    if (!hasAwardedGameAchievement) {
                        awardAchievement('maestro-panda', {
                            xp: 100,
                            sub: 'Completaste el Dojo del Panda · JLPT N2',
                            meta: { level: 'N2' },
                        }).catch((e) =>
                            console.error('Error otorgando logro Maestro Panda', e)
                        );
    ...
    const getPandaSource = () => {
        if (pandaState === 'fail') return pandaFail;
        if (pandaState === 'happy') return pandaHappy;
        ...
        return pandaWalkFrames[walkFrameIndex % pandaWalkFrames.length];
    };

    // 🐼 Animación de caminar (más lenta)
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setWalkFrameIndex((prev) => (prev + 1) % pandaWalkFrames.length);
        }, WALK_INTERVAL_MS);

        return () => clearInterval(interval);
    }, [pandaState, showIntroModal, showSummaryModal, showGameCompleteModal, timeLeft]);
    ...
    return (
        <View style={styles.screen}>
            ...
            >
                ...
                {/* 🐼 Panda con fondo suave */}
                <View style={styles.pandaBubble}>
                    <Image
                        source={getPandaSource()}
                        style={styles.panda}
                        resizeMode="contain"
                    />
                </View>
                        ...
                        <View style={styles.questionBackground}>
                            <Text style={styles.questionText}>
                                {currentQuestion
                                    ...
                                    : 'No hay más preguntas por ahora. ¡Has completado el contenido disponible para este nivel!'}
                            </Text>
                        </View>
                ...
                <Modal visible={showIntroModal} transparent animationType="fade">
                    <View style={styles.modalOverlay}>
                        <View style={styles.introModalContent}>
                            <Text style={styles.introTitle}>Dojo del Panda · JLPT N2</Text>
                            <Text style={styles.introSubtitle}>
                                Bienvenido al entrenamiento del Monje Panda
                            </Text>

                            <Text style={styles.introText}>
                                El monje panda te guía por el{' '}
                                <Text style={{ fontWeight: 'bold' }}>nivel JLPT N2</Text>.
                                Cada fase es un reto de vocabulario, gramática, lectura y
                                expresiones reales.
                            </Text>
                            ...
                            <Text style={styles.introPhrase}>
                                Completa todo el Dojo del Panda para obtener el logro{' '}
                                <Text style={{ fontWeight: 'bold' }}>
                                    “Maestro Panda” (100 XP)
                                </Text>
                                .
                            </Text>
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={styles.gameCompleteModalContent}>
                            ...
                            <Text style={styles.gameCompleteText}>
                                Has llegado al final del Dojo del Panda y completado todas las
